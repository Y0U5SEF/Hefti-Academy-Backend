...
async function generateMemberPdf(member, institution = {}) {
  // Return early in Vercel environment where Puppeteer won't work
  const isVercel = !!process.env.VERCEL
  if (isVercel) {
    // Return a simple PDF buffer with error message
    const errorMessage = `PDF generation not available in Vercel environment`
