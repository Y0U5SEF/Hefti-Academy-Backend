...
async function nextId(key) {
  ...
  if (res && res.value && typeof res.value.seq === 'number') return res.value.seq
  // Fallback in case driver did not return the document
  const doc = await db.collection('counters').findOne({ _id: key }, { projection: { seq: 1 } })
  if (doc && typeof doc.seq === 'number') return doc.seq
  // Last-resort init
  await db.collection('counters').updateOne({ _id: key, seq: { $exists: false } }, { $set: { seq: 1 } }, { upsert: true })
  return 1
}
